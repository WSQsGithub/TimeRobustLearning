...
task = 2; %'1: reachability, 2: patrolling'
problem = 2; % 1: maximize robust probability, 2: maximize worst case robustness
...
saveas(gcf,'results/task2p2_curve.fig','fig');
saveas(gcf,'results/task2p2_curve.png','png');
%% Evaluation 
rollout = 1000;
...
text(rollout-150,mean(srb)-0.3,strcat(['Pr=' num2str(mean(sat))]));
...
saveas(gcf,'results/task2p2_hist.fig','fig');
saveas(gcf,'results/task2p2_hist.png','png');
...
figure(1);
clf;hold on; grid on; xlim([0,grid_size]); ylim([0,grid_size]);
set(gca, 'XTick', 0:grid_size); set(gca, 'YTick', 0:grid_size);
rectangle('Position',[2,2,1,1],'edgecolor','k','facecolor','y','linewidth',1.8);  % mark goal
rectangle('Position',[1,3,1,1],'edgecolor','k','facecolor','y','linewidth',1.8);  % mark goal
plot(s_0(1),s_0(2),'Marker','p','MarkerSize',30,'MarkerEdgeColor','k','MarkerFaceColor','y','LineWidth',2);
plot(best_trace.trace(:,1),best_trace.trace(:,2),"Color",C.red,"LineWidth",2,'Marker','o','MarkerFaceColor',C.red);

saveas(gcf,'results/task2p2_sample_trace.png','png');
saveas(gcf,'results/task2p2_sample_trace.fig','fig');

figure(2);
subplot(311); hold on; set(gca, 'XTick',0:T+h);
% rectangle('Position',[0,1,T+h,1],'edgecolor','none','facecolor',[C.red 0.2],'linewidth',1.8);  % mark goal
% rectangle('Position',[0,2,T+h,1],'edgecolor','none','facecolor',[C.purple 0.2],'linewidth',1.8);
plot(1:T+h,best_trace.trace(:,1),"LineWidth",2,"Color",C.blue); ylabel('x position'); 

subplot(312); hold on; set(gca, 'XTick',0:T+h);
% rectangle('Position',[0,3,T+h,1],'edgecolor','none','facecolor',[C.red 0.2],'linewidth',1.8);  % mark goal
% rectangle('Position',[0,2,T+h,1],'edgecolor','none','facecolor',[C.purple 0.2],'linewidth',1.8);
plot(1:T+h,best_trace.trace(:,2),"LineWidth",2,"Color",C.blue); ylabel('y position'); 

subplot(313); hold on;

set(gca, 'XTick',0:T+h);
plot(1:T+h,best_trace.return,"LineWidth",2,"Color",C.blue);ylabel('step reward'); 

saveas(gcf,'results/task2p2_sample_trace_xyr.png','png');
saveas(gcf,'results/task2p2_sample_trace_xyr.fig','fig');

%% 100 traces
figure(3);
grid on; xlim([0,grid_size]); ylim([0,grid_size]); hold on;
set(gca, 'XTick', 0:grid_size); set(gca, 'YTick', 0:grid_size);
plot(s_0(1),s_0(2),'Marker','p','MarkerSize',30,'MarkerEdgeColor','k','MarkerFaceColor','y','LineWidth',2);
rectangle('Position',[2,2,1,1],'edgecolor','k','facecolor','y','linewidth',1.8);  % mark goal
rectangle('Position',[1,3,1,1],'edgecolor','k','facecolor','y','linewidth',1.8);  % mark goal
...
saveas(gcf,'results/task2p2_100traces.png','png');
saveas(gcf,'results/task2p2_100traces.fig','fig');
%% save data
save('results/task2p2Data',"Q","Log","Evaluation");